...
def tf_length(sequence):
    """

    :param sequence: 2d or 3d tempora tensor
    :return: 1d of sequence length
    """
    ''''#takes in a sequence, returns scalar value of nonzero values for last dimension (batch,timestep,dim==1)
    I think'''
    length = tf.reduce_sum(sequence, reduction_indices=1)
...
def lstm_layer(scope, n, input, mask, state=None, _reuse=None):
    """

    :param scope: name scope for lstm_layer
    :param n: hidden input dimension
    :param input: previous layer for input
    :param mask: 2d mask
    :param state: optional
    :return: output, last hidden state = (c,h)
    """
    """Accepts n = hidden dim, input, masked (3d) and optional state arg
    Returns output and hidden state of the network

    State dim = [batch, incoming n]"""
    with tf.variable_scope(scope, reuse=_reuse):
        lstm_cell = tf.nn.rnn_cell.LSTMCell(n, state_is_tuple=True)
        if state is not None:
            # assumes state is already stock and has not been seperated
            # state = tf.nn.rnn_cell.LSTMStateTuple(state[0], state[1])
            val, state = tf.nn.dynamic_rnn(lstm_cell, input, dtype=tf.float32, sequence_length=tf_length(mask),
                                           initial_state=state)
        else:
            val, state = tf.nn.dynamic_rnn(lstm_cell, input, dtype=tf.float32, sequence_length=tf_length(mask))
        return val, state
...
def pool(max_time, n, input, mask):
    # TODO doesn't work witrh variable length batches
    """

    :param max_time: longest sequence length
    :param n: hidden layer
    :param input: I've forgotten
    :param mask: mask (3d)
    :return:
    """
    mask = tf.reshape(input, [-1, max_time / 2, n * 2])
    mask = tf.reduce_max(mask, reduction_indices=2)
    return (tf.reshape(input, [-1, max_time / 2, n * 2]), mask)


def w_layer(_name, shape):
    """

    :param _name: name for W layer
    :param shape: input/output dimensions
    :return: tensor
    """
    input, output = shape
    W = tf.get_variable(name=_name, initializer=glorot_uniform([input, output]))
    return W


def glorot_uniform(shape, type='sigmoid'):
    '''

    :param shape: tuple of input
    :param type: what activation method used (impacts init methods)
    :return: tensor of 2d initialization
    '''
    r = np.sqrt(6. / (shape[0] + shape[1]))
    return tf.random_uniform([shape[0], shape[1]], -r, r)


def get_last_relevant_timestep(output, length):
    """

    :param output: rnn outputs
    :param length: lenths of sequences (scalar)
    :return: last time step of the sequence
    """
...
def dense(n, incoming, init='glorot'):
    W = get_variable_shape(incoming)


def get_relevant_timesteps(tensor, mask):
    # TODO implement to get all timesteps (see notes on desk)
    return tensor
...
def get_fans(shape):
    ...
    :param shape: Shape of tensor being initiated
    :return: fan_in,fan_out for initialization
    """
    if len(shape) == 2:
        fan_in = shape[0]
        fan_out = shape[1]
...
def time_loss(s1, s2, mask_t, dropout=None):
    """

    :param s1: tensor sequence 1, 3D
    :param s2:  tensor sequence 2, 3d
    :param mask_t: relevant time steps of sequence
    :return: scalar MSE * magnitude of allowed gates
    """
    '''Input sequence1, sequence 2, ,masking (3dim), returns scalar mean
    For autoencoding loss! sum xi - x_hat_i'''
    length = tf.reduce_sum(mask_t, reduction_indices=-1)

    mse = tf.squared_difference(s1, s2)

    mse = tf.reduce_sum(mse, reduction_indices=2)
    mse = tf.mul(mse, mask_t)
    mse = tf.reduce_sum(mse, reduction_indices=1)
    mse /= tf.reduce_sum(mask_t, reduction_indices=1)

    tf.scalar_summary('mse', tf.reduce_mean(mse))
...
def pad_time_tf(input):
    input_flat = tf.reshape(input, [-1])
    zero_pad = tf.zeros([1])

def tf_get_max(x, axis=None, keepdims=False):
    """
    Max value along a tensor
    :param x:
    :param axis:
    :param keepdims:
    :return:
